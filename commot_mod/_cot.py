...
def cot_combine_sparse(S, D, A, M, cutoff, M_mod=None, eps_p=1e-1, eps_mu=None, eps_nu=None, rho=1e1, weights=(1.00,0.00,0.00,0.00), nitermax=1e4, stopthr=1e-8, verbose=False):
...
def cot_sparse(S, D, A, M, cutoff, M_mod=None, eps_p=1e-1, eps_mu=None, eps_nu=None, rho=1e1, nitermax=1e4, stopthr=1e-8, verbose=False):
    ...
    for i in range(ns_s):
        for j in range(ns_d):
            if not np.isinf(A[i,j]):
                ...
                cost_vals = tmp_M_max_sp.data[tmp_ind] * A[i, j]
                C_data.append(cost_vals)
                C_row.append(tmp_row + i * n_pos_s)
                C_col.append(tmp_col + j * n_pos_d)

                # 建議用「調節後的成本」來估計 scale，較一致
                if cost_vals.size:
                    cost_scales.append(cost_vals.max())
                has_edge = True

    cost_scale = (max(cost_scales) if len(cost_scales) > 0 else 1.0)   #原 cost_scale = np.max(cost_scales)
...
def cot_row_sparse(S, D, A, M, cutoff, M_mod=None, eps_p=1e-1, eps_mu=None, eps_nu=None, rho=1e1, nitermax=1e4, stopthr=1e-8, verbose=False):
    """Solve for each sender species separately.
    """
    if eps_mu is None: eps_mu = eps_p
    if eps_nu is None: eps_nu = eps_p
    ...
    if M_mod is not None:
        exp_shape = (n_pos_s, ns_s + ns_d)
        if M_mod.shape != exp_shape:
            raise ValueError(f"M_mod.shape must be {exp_shape}, got {M_mod.shape}")
    ...
    for i in range(ns_s):
        ...
        for j in range(len(D_ind)):
            ...
            tmp_col = tmp_nzind_d[tmp_M_max_sp.col[tmp_ind]]
            # 距離 × 物種權重
            cost_vals = tmp_M_max_sp.data[tmp_ind] * A[i, D_j]

            # --- NEW: 若有提供 M_mod，按 (k,i) 與 (l,j) 做調節 ---
            # M_mod 形狀 = (n_cells, ns_s + ns_d)
            if M_mod is not None and cost_vals.size:
                # tmp_row = sender cell 索引 k；tmp_col = receiver cell 索引 l
                Ms = M_mod[tmp_row, i]              # sender 側 (k,i)
                Mr = M_mod[tmp_col, ns_s + D_j]     # receiver 側 (l,j) —— 右半偏移 ns_s
                M_fac = Ms * Mr
                M_fac = np.maximum(M_fac, 1e-12)    # 保底，避免除 0
                cost_vals = cost_vals / M_fac       # 等價於 C̃ = C / M
...
def cot_col_sparse(S, D, A, M, cutoff, M_mod=None, eps_p=1e-1, eps_mu=None, eps_nu=None, rho=1e1, nitermax=1e4, stopthr=1e-8, verbose=False):
    """Solve for each destination species separately.
    """
    if eps_mu is None: eps_mu = eps_p
    if eps_nu is None: eps_nu = eps_p
    ...
    for j in range(ns_d):
        ...
        for i in range(len(S_ind)):
            ...
            tmp_row = tmp_nzind_s[tmp_M_max_sp.row[tmp_ind]]
            tmp_col = tmp_nzind_d[tmp_M_max_sp.col[tmp_ind]]
            cost_vals = tmp_M_max_sp.data[tmp_ind] * A[S_i, j]
            # --- NEW: 用單一矩陣 M_mod 調節 (k,i) 與 (l,j) ---
            # M_mod 形狀 = (n_cells, ns_s + ns_d)
            if M_mod is not None and cost_vals.size:
                # tmp_row = sender cell 索引 k；tmp_col = receiver cell 索引 l
                Ms = M_mod[tmp_row, S_i]           # sender 側調節 (k,i)
                Mr = M_mod[tmp_col, ns_s + j]      # receiver 側調節 (l,j) —— 右半偏移 ns_s
                M_fac = Ms * Mr
                M_fac = np.maximum(M_fac, 1e-12)   # 保底，避免除 0
                cost_vals = cost_vals / M_fac      # 等價於 C̃ = C / M
            ...
            if cost_vals.size:
                cost_scales.append(cost_vals.max())
        cost_scale = (max(cost_scales) if cost_scales else 1.0)
        C_data = np.concatenate(C_data, axis=0)
        C_row = np.concatenate(C_row, axis=0)
        C_col = np.concatenate(C_col, axis=0)
        C = sparse.coo_matrix((C_data/cost_scale, (C_row, C_col)), shape=(len(a), len(b)))    
...
def cot_blk_sparse(S, D, A, M, cutoff, M_mod=None, eps_p=1e-1, eps_mu=None, eps_nu=None, rho=1e1, nitermax=1e4, stopthr=1e-8, verbose=False):
    if eps_mu is None: eps_mu = eps_p
    if eps_nu is None: eps_nu = eps_p
    if max(abs(eps_p-eps_mu), abs(eps_p-eps_nu)) > 1e-8:
        unot_solver = "momentum"
    ...
    for i in range(ns_s):
        for j in range(ns_d):
            if not np.isinf(A[i,j]):
                ...
                # 距離 × L–R 權重
                cost_vals = tmp_M_max_sp.data[tmp_ind] * A[i, j]
                # --- NEW: 用 M_mod 調節每條 (k,l) 邊 ---
                # M_mod 形狀 = (n_cells, ns_s + ns_d)
                if M_mod is not None and cost_vals.size:
                    # tmp_row=sender cell 索引 k；tmp_col=receiver cell 索引 l
                    Ms = M_mod[tmp_row, i]            # sender 側 (k,i)
                    Mr = M_mod[tmp_col, ns_s + j]     # receiver 側 (l,j) —— 右半偏移 ns_s
                    M_fac = Ms * Mr
                    M_fac = np.maximum(M_fac, 1e-12)  # 保底
                    cost_vals = cost_vals / M_fac     # 等價於 C̃ = C / M

                # 用調節後的成本來決定 scale，比較一致
                cost_scale = (cost_vals.max() if cost_vals.size else 1.0)
                C = sparse.coo_matrix((cost_vals / cost_scale, (tmp_row, tmp_col)),
                                    shape=(len(a), len(b)))
